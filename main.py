...
cadastrados = [
    ...
    Aluno(555, 'Bruce Wayne Batman', 4, 10.0, [], ["COMP359", "COMP360","COMP361", "COMP362", "COMP363", "COMP365", "COMP366","COMP364", "COMP367"], [], 0),
    Aluno(666, 'Lucifer Morningstar', 2, 9.0, [], ["COMP359", "COMP360", "COMP361", "COMP362", "COMP363", "COMP364", "COMP365", "COMP366", "COMP367", "COMP368", "COMP369", "COMP370", "COMP371", "COMP372", "COMP373", "COMP374", "COMP376", "COMP378", "COMP379", "COMP380", "COMP381", "COMP382", "COMP386", "COMP387", "COMP389", "COMP390", "COMP391", "COMP392", "COMP393", "COMP394", "COMP395", "COMP396", "COMP397", "COMP398", "COMP399", "COMP400", "COMP401"], [], 0),
    Aluno(888, 'José Henrique da Silva', 4, 6.0, [], ["COMP359","COMP365","COMP360","COMP361", "COMP362", "COMP363", "COMP365", "COMP366", "COMP367"], ["COMP364"], 0),
]
                            ...
                            if grade_CC[sel].pre_requisitos != []:
                                for i in range(len(grade_CC[sel].pre_requisitos)):
                                    if not grade_CC[sel].pre_requisitos[i] in aluno.materias_pagas:
                                        print("Não cumpre os pré-requisitos para ingressar na matéria. Você precisa pagar:")
                    ...
                    if materia in grade_TCC:
                            ...
                            for horarios_atuais_materias in grade_externa[materia].horario: #pega essa string materia e acessa seus horarios
                                for horarios_materias in grade_externa[escolha].horario: #loopa os horario da materia escolhida com a materia que possui
                                    if horarios_atuais_materias == horarios_materias:
                                        check=1
                                        print(f"Você não tem horário disponível para essa matéria, o horário da diciplina {grade_externa[materia].nome} é conflitante com a que voce quer aplicar ({grade_externa[escolha].nome}).")
                                        break
                                if check == 1:   
                                    break
                        else:
                            for horarios_atuais_materias in grade_CC[materia].horario: #pega essa string materia e acessa seus horarios
                                for horarios_materias in grade_externa[escolha].horario: #loopa os horario da materia escolhida com a materia que possui
                                    if horarios_atuais_materias == horarios_materias:
                                        check=1
                                        print(f"Você não tem horário disponível para essa matéria, o horário da diciplina {grade_externa[materia].nome} é conflitante com a que voce quer aplicar ({grade_externa[escolha].nome}).")
                        ...
                        for horarios_atuais_materias in grade_CC[materia].horario: #pega essa string materia e acessa seus horarios
                            for horarios_materias in grade_CC[escolha].horario: #loopa os horario da materia escolhida com a materia que possui
                                if horarios_atuais_materias == horarios_materias:
                                    check=1
                                    print(f"Você não tem horário disponível para essa matéria, o horário da diciplina {grade_CC[materia].nome} é conflitante com a que voce quer aplicar ({grade_CC[escolha].nome}).")
                                    break
                            if check == 1:   
                                break
                    if check == 1:
                    ...
                    if materia in grade_TCC:
                        ...
                        if materia in grade_externa:
                            for horarios_atuais_materias in grade_externa[materia].horario: #pega essa string materia e acessa seus horarios
                                for horarios_materias in grade_externa[escolha_insercao].horario: #loopa os horario da materia escolhida com a materia que possui
                                    if horarios_atuais_materias == horarios_materias:
                                        check=1
                                        print(f"Você não tem horário disponível para essa matéria, o horário da diciplina {grade_externa[materia].nome} é conflitante com a que voce quer aplicar ({grade_externa[escolha_insercao].nome}).")
                                        break
                                if check == 1:   
                                    break
                        else:
                        ...
                        for horarios_atuais_materias in grade_CC[materia].horario: #pega essa string materia e acessa seus horarios
                            for horarios_materias in grade_CC[escolha_insercao].horario: #loopa os horario da materia escolhida com a materia que possui
                                if horarios_atuais_materias == horarios_materias:
                                    check=1
                                    print(f"Você não tem horário disponível para essa matéria, o horário da diciplina {grade_CC[materia].nome} é conflitante com a que voce quer aplicar ({grade_CC[escolha_insercao].nome}).")
                                    break
                            if check == 1:   
                                break
                    if check == 1:
...
def exibir_Alunos():
    for i in range(len(matriculados)):
        print("\n#", str(matriculados[i].id) + " || "+"\033[1m"+"Nome: "+"\033[0m" + matriculados[i].nome + " || "+"\033[1m"+"matérias atuais: "+"\033[0m" + str(
            matriculados[i].materias_atuais) + " || "+"\033[1m"+"coeficiente: "+"\033[0m" + str(
            matriculados[i].coeficiente) +  " || "+"\033[1m"+"fluxo: "+"\033[0m" + str(matriculados[i].fluxo) +  " || "+"\033[1m"+"matérias pagas:"+"\033[0m" + str(
            matriculados[i].materias_pagas) + " || "+"\033[1m"+"pendências: "+"\033[0m" + str(matriculados[i].pendencias))
    return
